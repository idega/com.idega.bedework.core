...
public class Filters implements Serializable {
  ...
  /* True if somewhere in the query we specify a collection. Allows us to limit
   * queries to the current user if none is specified.
   */
  private boolean collectionsSpecified = false;
   ...
   */
  public boolean getCollectionsSpecified() {
    return collectionsSpecified;
  }
   ...
   * @throws CalFacadeException
   */
  public boolean postFilter(final BwEvent ev) throws CalFacadeException {
    return match(fullFilter, ev);
  }
  ...
  private boolean match(final FilterBase f,
    ...
    try {
      if (pf instanceof BwCategoryFilter) {
        return traceMatch(pf.match(ev)) ;
      }

      if (f instanceof PresenceFilter) {
        return traceMatch(matchPresence(pf.getPropertyIndex(), ev));
      }
  ...
  private boolean matchPresence(final PropertyInfoIndex pi,
    ...
    case DURATION:
      return ev.getEndType() == BwEvent.endTypeDuration;
    ...
    case DTEND:
      return ev.getEndType() == BwEvent.endTypeDate;

    case TRANSP:
      return ev.getTransparency() != null;
