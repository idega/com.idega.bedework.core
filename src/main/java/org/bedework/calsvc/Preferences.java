...
class Preferences extends CalSvcDb implements PreferencesI {
  ...
  public void updateAdminPrefs(final boolean remove,
    ...
    if (cat != null) {
      if (!remove) {
        ...
        sess.namedQuery("removeCategoryPrefForAll");
    ...
    if (loc != null) {
      if (!remove) {
        ...
        sess.namedQuery("removeLocationPrefForAll");
    ...
    if (ctct != null) {
      if (!remove) {
        ...
        sess.namedQuery("removeContactPrefForAll");

        sess.setInt("id", ctct.getId());
  ...
  public String getAttachmentsPath() throws CalFacadeException {
    ...
    if (path == null) {
      path = getSvc().getCalendarsHandler().getHome() + "/" + "attachments";
  ...
  private BwPreferences fetch(final BwPrincipal principal) throws CalFacadeException {
    ...
    sess.setString("ownerHref", principal.getPrincipalRef());
    sess.cacheableQuery();

    BwPreferences prefs = (BwPreferences)sess.getUnique();
