...
class Synch extends CalSvcDb implements SynchI {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1460938654609835794L;
	...
	Synch(final CalSvc svci, final BwUser user) {
		super(svci, user);
	}
	 ...
	 * 
	 * @see
	 * org.bedework.calsvci.SynchI#subscribe(org.bedework.calfacade.BwCalendar)
	 */
	@Override
	public boolean subscribe(final BwCalendar val) throws CalFacadeException {
		...
		// We'll make this up for the moment - needs to be handed to us by front
		// end
		...
		aosA.getProperty()
				.add(makeSynchProperty("principal", getPrincipal()
						.getPrincipalRef()));
		...
		if (val.getRemoteId() != null) {
			aosB.getProperty().add(
					makeSynchProperty("principal", val.getRemoteId()));
			aosB.getProperty().add(
					makeSynchProperty("password", val.getRemotePw()));
		}
		...
		aosB.getProperty().add(
				makeSynchProperty("refreshDelay",
						String.valueOf(refreshRate * 10000)));
		...
		aos.getProperty().add(
				makeSynchProperty("scheduling-processing", "REMOVE"));
...
//		sresp.setStatus(StatusType.OK);
	...
	public boolean unsubscribe(final BwCalendar val) throws CalFacadeException {
		...
		UnsubscribeRequestType usreq = (UnsubscribeRequestType) makeAsr(val,
				new UnsubscribeRequestType(), sc.getSynchToken());
...
//		usresp.setStatus(StatusType.OK);
	...
	public CheckSubscriptionResult checkSubscription(final BwCalendar val)
		...
		if (val.getSubscriptionId() != null) {
			...
			SubscriptionStatusRequestType ssreq = (SubscriptionStatusRequestType) makeAsr(
					val, new SubscriptionStatusRequestType(),
					sc.getSynchToken());

			SubscriptionStatusResponseType ssresp = getPort(getSynchUri())
					.subscriptionStatus(
	...
	public BwSynchInfo getSynchInfo() throws CalFacadeException {
		...
		if ((girt == null) || (girt.getInfo() == null)) {
			throw new CalFacadeException("Unable to fetch synch info");
		}
	...
	private ActiveSubscriptionRequestType makeAsr(final BwCalendar val,
			final ActiveSubscriptionRequestType asr, final String synchToken)
			throws CalFacadeException {
		if (val.getSubscriptionId() == null) {
	...
	SynchIdTokenType getIdToken(final String principal, final SynchConnection sc) {
	...
	SynchRemoteServicePortType getPort(final String uri)
		...
		try {
			...
			SynchRemoteService ers = new SynchRemoteService(wsURL,
					synchServicename);
