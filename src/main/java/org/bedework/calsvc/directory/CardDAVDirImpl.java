...
//        Attributes attrs = sr.getAttributes();
      ...
      for (String mbr: mbrs) {
        ...
        if (response.hasMore()) {
          ...
          if (isGroup) {
            ...
            groupMbr.setPrincipalRef(makePrincipalUri(groupMbr.getAccount(),
                                                      WhoDefs.whoTypeGroup));
            ...
            user.setPrincipalRef(makePrincipalUri(user.getAccount(),
                                                  WhoDefs.whoTypeUser));
   ...
   */
  private String getUserEntryValue(final LdapConfigProperties props, final BwUser user) {
    return makeUserDn(props, user);
  }

  /* Return the entry we will find in a group identifying this group
   */
  private String getGroupEntryValue(final LdapConfigProperties props, final BwGroup group) {
    return makeGroupDn(props, group);
  }

  private String makeUserDn(final LdapConfigProperties props, final BwUser user) {
    return props.getUserDnPrefix() + user.getAccount() +
           props.getUserDnSuffix();
  }

  private String makeGroupDn(final LdapConfigProperties props, final BwGroup group) {
