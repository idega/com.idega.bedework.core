...
public abstract class SchedulingBase extends CalSvcDb implements SchedulingIntf {
  ...
  */
  ...
  protected boolean significantChange(final EventInfo ei) throws CalFacadeException {
    if (ei.getNewEvent() ||
        (ei.getChangeset() == null) ||
        ei.getChangeset().getSignificantChange()) {
      return true;
    }
  ...
  protected EventInfo copyEventInfo(final EventInfo ei,
    ...
    boolean significant = (ei.getChangeset() == null) ||
                           ei.getChangeset().getSignificantChange();
      ...
      for (EventInfo oei: overrides) {
        ...
        if (!masterSuppressed && attendeeInMaster && attendeeInOverride) {
          ...
          if (significantChangesOnly) {
            significant = (oei.getChangeset() == null) ||
                           oei.getChangeset().getSignificantChange();
  ...
  protected BwEvent copyEvent(final BwEvent origEv, final BwEvent masterEv,
                              final BwPrincipal owner) throws CalFacadeException {
    BwEvent newEv;
  ...
  protected boolean initScheduleEvent(final EventInfo ei,
                                      final boolean response) throws CalFacadeException {
    BwEvent event = ei.getEvent();
