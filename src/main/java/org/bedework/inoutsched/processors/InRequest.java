...
public class InRequest extends InProcessor {
  ...
  public ProcessResult process(final EventInfo ei) throws CalFacadeException {
    ...
    String uri = getSvc().getDirectories().principalToCaladdr(getSvc().getUser());
    String colPath = null;
  ...
  private boolean checkBusy(final CalSvcI svci,
    ...
    BwEvent fb = svci.getScheduler().getFreeBusy(null, svci.getUser(),
                                                 start, end, org,
                                                 uid, excludeUid);
  ...
  private EventInfo newAttendeeCopy(final CalSvcI svci,
    ...
    // Adding a copy
    EventInfo calEi = sched.copyEventInfo(inEi, svci.getUser());
    ...
    if (att == null) {
      ...
      if (debug) {
        trace("Schedule - no attendee with uri " + uri +
              " for " + svci.getUser().getPrincipalRef());
      }
  ...
  private boolean updateAttendeeCopy(final EventInfo ourCopy,
    ...
    ChangeTable chg = ourCopy.getChangeset();

    if (chg == null) {
      chg = new ChangeTable();
      ourCopy.setChangeset(chg);
    }
